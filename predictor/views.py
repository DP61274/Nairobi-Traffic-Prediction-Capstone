...
def chatbot_predict(request):
    try:
        ...
        if not payload.get('to_lat') and not payload.get('destination'):
            return JsonResponse({
                'ok': False,
                'message': "I need a destination before I can predict the route.",
            }, status=400)

        if not payload.get('from_lat') and not payload.get('origin'):
            return JsonResponse({
                'ok': False,
                'message': "I need a starting location before I can predict the route.",
            }, status=400)
